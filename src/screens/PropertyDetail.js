    ...
    <SafeAreaView className="flex-1 bg-white dark:bg-gray-950" edges={["bottom"]}>
      <ScrollView>
        ...
        <View className="p-4">
          ...
          <Text className="text-gray-500 dark:text-gray-400 mt-1">{property.city}</Text>
          <Text className="font-bold text-xl text-brand-700 dark:text-brand-400 mt-3">
            {formatPrice(property.price)}
          </Text>

          <View className="flex-row gap-2 mt-4">
            {property.rooms != null && (
              <View className="bg-gray-100 dark:bg-gray-800 rounded-full px-3 py-1.5">
                <Text className="text-gray-700 dark:text-gray-300 text-sm">{property.rooms} hab.</Text>
              </View>
            )}
            {property.baths != null && (
              <View className="bg-gray-100 dark:bg-gray-800 rounded-full px-3 py-1.5">
                <Text className="text-gray-700 dark:text-gray-300 text-sm">{property.baths} baños</Text>
              </View>
            )}
            {property.parking != null && (
              <View className="bg-gray-100 dark:bg-gray-800 rounded-full px-3 py-1.5">
                <Text className="text-gray-700 dark:text-gray-300 text-sm">{property.parking} parqueos</Text>
              </View>
            )}
          </View>

          {property.description ? (
            <Text className="text-gray-700 dark:text-gray-300 mt-4 leading-6">{property.description}</Text>
          ) : null}

          {property.publishedBy ? (
            <View className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-800">
          ...
          {!property.lat && (
            <Pressable className="mt-4">
              <Text className="text-brand-700 dark:text-brand-400 font-medium">
